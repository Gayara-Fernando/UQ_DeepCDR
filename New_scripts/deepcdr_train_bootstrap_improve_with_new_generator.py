...
def run(params: Dict):
    ...
    val_gen_bootstraps = DataGenerator(valid_gcn_feats, valid_adj_list, valid_keep["Cell_Line"].values.reshape(-1,1), valid_keep["Cell_Line"].values.reshape(-1,1),
    ...
    for i in range(1, B+1):
        ...
        # ------------------------------------------------------
        modelpath = frm.build_model_path(model_file_name = params["model_file_name"], model_file_format = params["model_file_format"], model_dir = folder_loc)
        ...
        # save the model
        check.save(modelpath)

        # load the model again for evaluation?
        check = tf.keras.models.load_model(modelpath)
    
        # evaluate the model
        y_val_preds, y_val_true = batch_predict(check, val_gen_bootstraps)
