...
modelpath = frm.build_model_path(model_file_name="DeepCDR_model", model_file_format="", model_dir="new_gen_results")
print(modelpath)
...
val_gen_alt = DataGenerator(valid_gcn_feats, valid_adj_list, valid_keep["Cell_Line"].values.reshape(-1,1), valid_keep["Cell_Line"].values.reshape(-1,1), valid_keep["Cell_Line"].values.reshape(-1,1), valid_keep["AUC"].values.reshape(-1,1), batch_size=32, shuffle = False)
...
check.fit(train_gen_alt, validation_data = val_gen_alt, epochs = epoch_num, steps_per_epoch=steps_per_epoch, validation_steps=validation_steps,
...
# save the model?
check.save(os.path.join(modelpath, "DeepCDR_model_with_data_generator"))
    
# get the predictions on the validation set
val_steps = int(np.ceil(len(valid_gcn_feats) / generator_batch_size))
preds_val, target_val = batch_predict(check, val_gen_alt, val_steps)  
...
# Okay, we have a working code with the new data generator

# now what if we have these functions - which are necessary for the batching and predictive batching in a separate python script?
# Can we do that? Let's figure it out - paste these functions in a separate script, and call these in the imports
