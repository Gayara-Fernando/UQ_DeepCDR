...
def run(params: Dict):
    ...
    train_data_gen = DataGenerator(train_gcn_feats, train_adj_list, train_keep["Cell_Line"].values.reshape(-1,1),
                                   train_keep["Cell_Line"].values.reshape(-1,1), train_keep["Cell_Line"].values.reshape(-1,1),
                                   train_keep["AUC"].values.reshape(-1,1), batch_size=32,  shuffle = False)

    val_data_gen = DataGenerator(valid_gcn_feats, valid_adj_list, valid_keep["Cell_Line"].values.reshape(-1,1),valid_keep["Cell_Line"].values.reshape(-1,1),
                                 valid_keep["Cell_Line"].values.reshape(-1,1), valid_keep["AUC"].values.reshape(-1,1), batch_size=32,  shuffle = False)

    test_data_gen = DataGenerator(test_gcn_feats, test_adj_list, test_keep["Cell_Line"].values.reshape(-1,1), test_keep["Cell_Line"].values.reshape(-1,1),
    ...
    for i in range(1, B + 1):
        ...
        # joined path
        folder_loc = os.path.join(folder_path, folder_nm)
        # ------------------------------------------------------
    ...
    ########### Training the NNE model ##############

    training = False
    dropout1 = 0.10
    dropout2 = 0.20
    NNE_model = deepcdrgcn_NNe(dict_features, dict_adj_mat, samp_drug, samp_ach, cancer_dna_methy_model, cancer_gen_expr_model, cancer_gen_mut_model,
                               training = training, dropout1 = dropout1, dropout2 = dropout2)
    ...
    train_gen_NNe = DataGenerator(train_gcn_feats, train_adj_list,train_keep["Cell_Line"].values.reshape(-1,1),train_keep["Cell_Line"].values.reshape(-1,1),
                                  train_keep["Cell_Line"].values.reshape(-1,1), r_2_true_train, batch_size=32)
    val_gen_NNe = DataGenerator(valid_gcn_feats, valid_adj_list, valid_keep["Cell_Line"].values.reshape(-1,1), valid_keep["Cell_Line"].values.reshape(-1,1),
                                valid_keep["Cell_Line"].values.reshape(-1,1), r_2_true_valid, batch_size=32,  shuffle = False)
    ...
    test_gen_NNe = DataGenerator(test_gcn_feats, test_adj_list, test_keep["Cell_Line"].values.reshape(-1,1), test_keep["Cell_Line"].values.reshape(-1,1),
                                 test_keep["Cell_Line"].values.reshape(-1,1), r_2_true_test, batch_size=32,  shuffle = False)
    # compile the model
    lr = 0.01
    # compile the model, and use the new custom loss function
    NNE_model.compile(loss = lambda y_true, y_pred: correct_custom_loss(y_true, y_pred), 
...
def main(args):
    ...
    params = cfg.initialize_parameters(
        ...
        default_config = "deepcdr_params_bootstraps.txt",
